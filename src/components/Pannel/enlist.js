const cnlist = {
  v1: {
    name: "Jademont Zheng(Host)",
    position: "Committee Member of BitShares, Committee Member of Waterdrip Capital"
  },
  v2: {
    name: 'Jerry Liu(Bitcrab)',
    position: 'Founder of GDEX.IO  Committee Member of BitShares'
  },
  v3: {
    name: 'Fabian Schuh(Xeroc)',
    position: 'CTO of Blockchain Projects'
  },
  v4: {
    name: 'Ronny Boesing',
    position: 'CEO and Co-founder of OpenLedger ApS. Committee member of BitShares'
  },
  v5: {
    name: 'Jonathan Bahai',
    position: 'Founder of Peerplays'
  },
  v6: {
    name: 'Annemieke Dirkes',
    position: 'Official Public Face and sole Spokesperson of the BitShares Blockchain',
  },
  v7: {
    name: 'Ryan R. Fox',
    position: 'Coordinator of BitShares Core Team'
  },  
  v8: {
    name: 'Qiang Liu',
    position: 'Founder of hellobts'
  },
  v9: {
    name: 'Zicen',
    position: 'Committee Member of BitShares, Co-founder of YOYOW'
  },
  v10: {
    name: 'Huabo Hu',
    position: 'Founder of Wine, Co-founder of Token++ wallet'
  }
}

export default cnlist
