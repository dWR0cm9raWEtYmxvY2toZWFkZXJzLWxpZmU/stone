const list = {
  v1: {
    name: 'Jerry Liu',
    position: 'Founder of GEDX',
		position2: 'Committee Member of BitShares'
  },
  v2: {
    name: 'Stan Larimer',
    position: 'CEO of Cryptonomex',
  },
  v3: {
    name: 'Ronny Boesing',
    position: 'Founder and CEO of OpenLedger ApS',
		position2: 'Committee Member of BitShares'
  },
  v4: {
    name: 'Annemieke Dirkes',
    position: 'Official Public Face',
		position2: 'sole Spokesperson of the BitShares Blockchain'
  },
  v5: {
    name: 'Qiang Liu',
    position: 'Founder of hellobts'
  },
  v6: {
    name: 'Harvey Zhao',
    position: 'Founder and CEO of OracleChain'
  },	
  v7: {
    name: 'Mark Babbitt',
    position: 'CCO of Crypviser GmbH'
  },
  v8: {
    name: 'Branislav Zelenak',
    position: 'Blockchain Team Leader of DasCoin'
  },
  v9: {
    name: 'Damir Nedžibović',
		position: 'Senior Blockchain developer of DasCoin'
  },
	v10: {
		name: 'Vlad Sapozhnikov',
		position: 'Co-founder and CEO of deex.exchange'
	},
  v11: {
    name: 'Ivan Bandaryk',
    position: 'CTO of OpenLedger',
		position2: 'BitShares witness OpenLedger-dc'
  },	
  v12: {
    name: 'Zicen',
		position: 'Founder of HelloEOS',
    position: 'Committee Member of BitShares',
		position2: 'Co-founder of YOYOW'
  },
  v13: {
    name: 'Zengbo Zhang',
    position: 'CTO of Magicwallet'
  },
	v14: {
		name: 'Yury Parsamov',
		position: 'Gravity Protocol CEO'
	},
	v15: {
		name: 'Peter Asalkhanov',
		position: 'Head of Research, Gravity Protocol '
	},
  v16: {
    name: 'Matej Michalko',
    position: 'Founder and CEO of DECENT Foundation Co-founder of ALAX'
  },
  v17: {
    name: 'Fabian Schuh (Xeroc)',
    position: 'CTO of Blockchain Projects',
  },
  v18: {
    name: 'Ryan R. Fox',
    position: 'Coordinator of BitShares Core Team'
  },
  v19: {
    name: 'Minqiang Huang',
    position: 'Founder and CEO of GXS'
  },
  v20: {
    name: 'Alex Ye',
    position: 'CYBEX CTO',
  },
  v21: {
    name: 'Jonathan Bahai',
    position: 'Found of Peerplays',
		position2: 'eXeBlock Technologies Corp (CSE.XBLK)'
  },  
  v22: {
    name: 'Alfredo Garcia',
    position: 'BitShares Core Developer'
  },
  v23: {
    name: 'Christoph Hering',
    position: 'Founder and CEO of Payger'
  },  
  v24: {
    name: 'Yi Luo',
    position: 'Researcher of blockchain technology'
  },
  v25: {
    name: 'Alex Shkor',
    position: 'CTO at Scorum Founder of DEIP.world'
  },
  v26: {
    name: 'Sigve Kvalsvik',
    position: 'Developer of BitShares UI team'
  },
  v27: {
    name: 'Ocean Liao',
    position: 'Founder of EOS gravity'
  },
  v28: {
    name: 'Jademont Zheng',
    position: 'Committee Member of BitShares',
		position2: 'Committee Member of Waterdrip Capital'
  },
  v29: {
    name: 'Neo Ma',
    position: 'Technical geek'
  },
	v30: {
		name: 'Chao Wang',
		position: 'Co-founder and CTO of Bottos'
	},
  v31: {
    name: 'Lingyan Zhao',
    position: 'Founder of UnlimitedIP program',
		position2: 'CEO of Link Entertainments Technology'
  },
  v32: {
    name: 'Peter Wu',
    position: 'Founder and CEO of Chaince Trading Platform'
  },
  v33: {
    name: 'Ruiqi Shi',
    position: 'Co-Founder of EOS Cannon  Founder',
		position2: 'Director of @BYSTAKE',
		position3: '“胖哥说币”blogger owner'
  },
  v34: {
    name: 'Yahuang Wu',
    position: 'CTO of MEET.ONE'
  },
  v35: {
    name: 'Huabo Hu',
    position: 'Founder of Winex',
		position2: 'Co-founder of Token++ wallet'
  },
  v36: {
    name: 'Xuqiang Wu',
    position: 'Founder of btsabc.org'
  },
  v37: {
    name: 'Zongqiu Yao',
    position: 'Witness of BitShares Evangelist'
  }
}

export default list
